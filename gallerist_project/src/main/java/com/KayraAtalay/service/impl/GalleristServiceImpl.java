...
public class GalleristServiceImpl implements IGalleristService {
	...
	private Gallerist createGallerist(DtoGalleristIU dtoGalleristIU) {
		...
		if (optAddress.isEmpty()) {
			throw new BaseException(
					new ErrorMessage(MessageType.ADDRESS_NOT_FOUND, dtoGalleristIU.getAddressId().toString()));
		}
	...
	public DtoGallerist saveGallerist(DtoGalleristIU dtoGalleristIU) {
		...
		return DtoConverter.toDto(savedGallerist);
	}
	...
	public Page<DtoGallerist> findAllPageableDto(Pageable pageable) {
		...
		return galleristPage.map(DtoConverter::toDto);
	...
	public DtoGallerist findGalleristById(Long id) {
		...
		return DtoConverter.toDto(optGallerist.get());
	}
